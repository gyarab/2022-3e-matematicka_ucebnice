...
export function generatePexeso(X_axis, difficulty) {
    /*
    TODO -> vygeneruj javascriptí mapu s hodnotami do pexesa. Například map.set('6', '36') nebo map.set('144', '2*72'). Prostě cokoliv, co se rovná.
    TODO -> X_axis a Y_axis udává velikost pexesa (pokud uznáš za vhodné, tak udělej pouze čtverec tzn. smaž parametr Y_axis)
    TODO -> odkaz jak mapa funguje: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map
    ...
    if (typeof X_axis === "undefined" || typeof difficulty === "undefined")
        throw new Error('All parameters must be defined!')
    if (length === null || difficulty === null)
        throw new Error('There are not null parameters acceptable only!')
    if (isNaN(X_axis))
    ...
    if (isNaN(difficulty))
        throw new Error('The difficulty is not a number!')
    if (!isFinite(X_axis) || !isFinite(difficulty))
        throw new Error('It is required to have finite value of the parameter!')

    let pairs = new Map()
    if (difficulty === 1) {
        for (let i = 0; i < X_axis ** 2; i++) {
        ...
        for (let i = 0; i < X_axis ** 2; i++) {
            ...
            if (choice === 0) {
                ...
                let a = Math.floor(Math.random() * 16 + 5);//do petky je docela cringe    
                pairs.set(+"√" + a * a, a);
            }
        }
    } else {
