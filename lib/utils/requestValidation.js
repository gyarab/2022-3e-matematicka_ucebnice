 ...
 * Second parameter 'email' presents the user email prepared to use for authentication.
 * Third parameter 'method' represents the string value of a required method of the request. ('GET', 'POST', etc.)
 ...
 */
export async function isValidRequest(req, email, method, shouldAuthenticate=true) {
    ...
    if (shouldAuthenticate) {
        const valid = await isAuthorized(email)
        if (!valid) {
            return {
                ok: false,
                status: 401,
